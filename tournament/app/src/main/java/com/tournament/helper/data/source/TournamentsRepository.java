...
public class TournamentsRepository implements TournamentsDataSource {
  ...
  public void getTournament(@NonNull final String tournamentId, @NonNull final GetTournamentCallback callback) {
    ...
    mTournamentsFBDataSource.getTournament(tournamentId, new GetTournamentCallback() {
      ...
      public void onTournamentLoaded(Tournament Tournament) {
        ...
        mCachedTournaments.put(tournamentId, Tournament);
  ...
  private void refreshCache(List<Tournament> tournaments) {
    ...
    saveTournamentToCP(tournaments);
    mCacheIsDirty = false;
  }

  private void saveTournamentToCP(final List<Tournament> tournaments) {
    new AsyncTask<Void, Void, Void>() {

      @Override
      protected Void doInBackground(Void... params) {
        context.getContentResolver().delete(Contract.PATH_TOURNAMENTS_URI, null, null);
        for(Tournament tournament : tournaments) {
          ContentValues values = new ContentValues();
          values.put(Contract._ID, tournament.getId());
          values.put(Contract.COL_NAME_TEXT, tournament.getTitle());

          final Uri uri = context.getContentResolver().insert(Contract.PATH_TOURNAMENTS_URI, values);
        }
        return null;
      }
    }.execute();
